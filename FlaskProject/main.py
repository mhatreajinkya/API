...
def add_emp():
    try:
        ...
        if _name and _email and _phone and _address and request.method == 'POST':
            sqlQuery = "INSERT INTO rest_emp(id,name, email, phone, address) VALUES(%s, %s, %s, %s, %s)"
...
@app.route('/emp/<int:id>',endpoint='emp2')
def emp2(id):
	try:
		conn = mysql.connect()
...
@app.route('/emp',methods=['GET'],endpoint='emp')
def emp():
    try:
        conn = mysql.connect()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute("SELECT id,name,address from rest_emp")
